  ...
  if (token) {
    ...
    if (monthResult) { //返回true
      if (monthResult != "ErrorTimeOut") { //在时间范围内并且返回true 就提示成功
        reMindMsg.text = `🎉 ${data.getFullYear()}年${data.getMonth() + 1 }月${data.getDate()}日 蘑菇丁【${config.phone}的${monthResult}】成功！ 🎉`;
        reMindMsg.desp = `${config.phone}的${monthResult}`;
        //       msg ______    发送消息
        await remind(axios, config, reMindMsg);
      }
    } else {
      reMindMsg.text = `🎉 ${data.getFullYear()}年${data.getMonth() + 1}月${data.getDate()}日 蘑菇丁【${config.phone}的${monthResult}】❗️ ❗️ ❗️ ❗️ 🎉`;
    ...
    if (weeksResult) {
      if (weeksResult != "OUTTIME") {
        reMindMsg.text = `🎉 ${data.getFullYear()}年${data.getMonth() + 1}月${data.getDate()}日 蘑菇丁【${config.phone}的${weeksResult}】 🎉`;
        reMindMsg.desp = `${config.phone}的${weeksResult}`;
        //       msg ______    发送消息
        await remind(axios, config, reMindMsg);
      }
    } else {
      reMindMsg.text = `🎉 ${data.getFullYear()}年${data.getMonth() + 1}月${data.getDate()}日 蘑菇丁【${config.phone}的${weeksResult} ❗️ ❗️ ❗️ ❗️  】 🎉`;
      reMindMsg.desp = `${config.phone}的${weeksResult}`;
      //       msg ______    发送消息
      await remind(axios, config, reMindMsg);
    }
